...
export class PersonService {
  ...
  getPeoples(): Observable<Person[]> {
    return this.http.get<Person[]>(this.peoplesUrl)
      .pipe(
        tap(_ => this.log('fetched heroes')),
        catchError(this.handleError<Person[]>('getPeoples', []))
      );
  }
  ...
  searchPeoples(term: string): Observable<Person[]> {
    if (!term.trim()) {
      // if not search term, return empty person array.
      return of([]);
    }
    return this.http.get<Person[]>(`${this.peoplesUrl}/?name=${term}`).pipe(
      tap(x => x.length ?
         this.log(`found peoples matching "${term}"`) :
         this.log(`no peoples matching "${term}"`)),
      catchError(this.handleError<Person[]>('searchpeoples', []))
    );
  }
  ...
  addPerson(person: Person): Observable<Person> {
    return this.http.post<Person>(this.peoplesUrl, person, this.httpOptions).pipe(
  ...
  updatePerson(person: Person): Observable<any> {
    return this.http.put(this.peoplesUrl, person, this.httpOptions).pipe(
